...
FROM
    ...
    LEFT JOIN patient_identifier pid3 ON(
        pid3.patient_id = patient.patient_id
        AND pid3.identifier_type = 8
        AND pid2.voided = 0
    )
    ...
    INNER JOIN obs obs2 ON(
        obs2.person_id = smin.person_id
        AND obs2.concept_id = smin.concept_id
        AND obs2.obs_datetime = smin.first_date
        AND obs2.concept_id IN(165843)
    )
    LEFT JOIN concept_name cn2 ON(
        obs2.value_coded = cn2.concept_id
        AND cn2.locale = 'en'
